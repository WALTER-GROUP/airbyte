...
class TransporeonInsightsStream(HttpStream, ABC):
    ...
    def __init__(self, config: Mapping[str, Any], authenticator, **kwargs):
        ...
        self.lanes = self.parse_input_list(config['lanes']['lane']) if type(config['lanes']['lane']) is not bool and not config['lanes_lvl2']\
            else self.get_lanes(config, self.metric)
    ...
    def parse_input_list(self, lanes: str) -> List[dict]:
        """
    ...
    def match_lanes(self, all_lanes: list, parsed_lanes: list) -> List[dict]:
        """
    ...
    def get_lanes(self, config: Mapping[str, Any], metric: str) -> List[dict]:
        ...
        if type(config["lanes"]["lane"]) is bool:
            ...
            parsed_lanes = self.parse_input_list(config["lanes"])
